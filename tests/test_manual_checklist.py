...
from app.schemas.transactions import ProcessingStatus
...
@pytest.mark.asyncio
async def test_database_state_verification():
    """Verify database state for manual testing."""
    async with AsyncSessionLocal() as session:
        # Check tax rules exist
        tax_rules = await session.execute(select(TaxRule))
        tax_rules_list = tax_rules.scalars().all()
        assert len(tax_rules_list) > 0, "Tax rules not seeded"

        # Check P&L mappings exist
        pl_mappings = await session.execute(select(PLRowMapping))
        pl_mappings_list = pl_mappings.scalars().all()
        assert len(pl_mappings_list) > 0, "P&L mappings not seeded"

        # Report counts
        print(f"\nDatabase State:")
        print(f"- Tax Rules: {len(tax_rules_list)}")
        print(f"- P&L Mappings: {len(pl_mappings_list)}")

        # Check for test data
        transactions = await session.execute(select(Transaction))
        trans_count = len(transactions.scalars().all())
        ...
        if trans_count > 0:
            ...
            cat_count = len(categorized.scalars().all())
            print(f"- Categorized: {cat_count}/{trans_count}")
