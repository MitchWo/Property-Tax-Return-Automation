...
async def get_categorization_audit_report(
    ...
    # Sort by transaction type (income first, then expenses, then excluded) and P&L row
    def sort_key(x):
        # Priority order: income (0), expense (1), excluded (2), unknown (3)
        type_order = {
            'income': 0,
            'expense': 1,
            'excluded': 2,
        ...
        transaction_type = x.get('transaction_type', 'unknown')
        return (
            type_order.get(transaction_type, 3),  # First by type
            x['pl_row'] or 999,                    # Then by P&L row
            x['category_code']                     # Finally by category code
        )

    category_details.sort(key=sort_key)
        ...
        .where(
            and_(
                ...
                or_(
                    ...
                    Transaction.category_code.is_(None)
                )
            )
        )
