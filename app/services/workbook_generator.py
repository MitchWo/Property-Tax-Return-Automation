...
class WorkbookGenerator:
    ...
    def _build_profit_loss_sheet(self, ws: Worksheet, context: Dict[str, Any]):
        ...
        # Totals (Row 46)
        row = 46
        ws[f"K{row}"] = "=SUM(K34:K45)"
        ws[f"L{row}"] = "=SUM(L34:L45)"
        ws[f"M{row}"] = "=SUM(M34:M45)"
        ws[f"N{row}"] = "=SUM(N34:N45)"
        ws[f"O{row}"] = "=SUM(O34:O45)"
        ws[f"P{row}"] = "=SUM(P34:P45)"
        ...
        # PM Totals (Row 64)
        row = 64
        ws[f"K{row}"] = "=SUM(K52:K63)"
        ws[f"L{row}"] = "=SUM(L52:L63)"
        ws[f"M{row}"] = "=SUM(M52:M63)"
        for col in ["K", "L", "M"]:
            ws[f"{col}{row}"].style = "total"
